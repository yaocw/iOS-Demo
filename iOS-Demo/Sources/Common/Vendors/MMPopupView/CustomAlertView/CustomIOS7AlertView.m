...
//{
//    if (delegate != NULL) {
//        [delegate   :self clickedButtonAtIndex:[sender tag]];
//    }
//
...
#pragma mark - button响应函数
- (void)customIOS7dialogButtonTouchUpInside: (CustomIOS7AlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if (alertView.tag == 12516) {
        if (buttonIndex == 1) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"ChoosePickerView" object:nil];
        }
    }
    if (alertView.tag == 11516) {
        if (buttonIndex == 1) {
            [[NSNotificationCenter defaultCenter] postNotificationName:@"CustomIOS7AlertView101516" object:nil];
        }
    }
    [self close];
}
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 300, 88)];
    }
   ...
   // UIView *dialogContainer = [[UIView alloc] initWithFrame:CGRectMake((screenSize.width - dialogSize.width) / 2, (screenSize.height - dialogSize.height) / 2, dialogSize.width, dialogSize.height)];
      UIView *dialogContainer = [[UIView alloc] initWithFrame:CGRectMake(0, 64, dialogSize.width, dialogSize.height)];
