...
- (instancetype)initWithTitle:(NSString *)title items:(NSArray *)items
{
    ...
    if ( self )
    {
        ...
        if ( title.length > 0 )
        {
            ...
            self.titleButton.backgroundColor = config.backgroundColor;
            
            self.titleLabel = [UILabel new];
            [self.titleButton addSubview:self.titleLabel];
            [self.titleLabel mas_makeConstraints:^(MASConstraintMaker *make) {
                make.edges.equalTo(self.titleButton).insets(UIEdgeInsetsMake(config.innerMargin, config.innerMargin, config.innerMargin, config.innerMargin));
            }];
            ...
            lastAttribute = self.titleButton.mas_bottom;
        }
